<?php
    ...
    #   script also create a folder named 'temp' if it does not exist.              #
    #                                                                               #
...
// Parameters
$picture = !empty($_GET['img']) ? $_GET['img'] : false; // string   - picture file name
$newW    = !empty($_GET['w']) ? $_GET['w'] : 0;     // integer  - new width (optional)
$newH    = !empty($_GET['h']) ? $_GET['h'] : 0;     // integer  - new height (optional)
$crop    = !empty($_GET['crop']) ? true : false; // boolean  - if true, the original picture will fill in the rectangle. Otherwise, it will fit in the rectangle (optional)
$id      = !empty($_GET['id']) ? $_GET['id'] : false; // integer  - unique element id (recommended)
...
function imageProcess($picture, $type, $format, $width, $height, $newW, $newH, $imgW, $imgH, $crop, $id, $enlarge, $format, $bgColor, $alpha, $filename, $quality) {
    ...
    if ($format == 'jpg') {
        if ($quality > 0 && $quality <= 100) {
            imagejpeg($img, './temp/' . $filename, $quality);
        } else {
            imagejpeg($img, './temp/' . $filename);
        }
        header('Content-type: image/jpeg');
    } elseif ($format == 'gif') { 
        if ($alpha == 127)
            imagecolortransparent($img, $back);
        imagegif($img, './temp/' . $filename);
        header('Content-type: image/gif');
    } elseif ($format == 'png') {
        if ($quality > 0 && $quality <= 100) {
            $quality = 10 - (round($quality / 10));
            imagepng($img, './temp/' . $filename, $quality);
        } else {
            imagepng($img, './temp/' . $filename);
        }
...
if ((strtolower(substr($picture, -3)) == 'jpg') ||
    ...
    $filename = substr(basename($picture), 0, -4) .
        ...
        (($quality > 0 && $quality <= 100) ? '_q' . $quality : '') . '.' . $format;
    ...
    if (file_exists('./temp/' . $filename)) {
        ...
        $thumbDate   = filemtime('./temp/' . $filename);
        if ($pictureDate > $thumbDate) {
            imageProcess($picture, $type, $format, $width, $height, $newW, $newH, $imgW, $imgH, $crop, $id, $enlarge, $format, $bgColor, $alpha, $filename, $quality);
        }
        
        // Check the cache of the browser
        $gmt  = gmdate('D, d M Y H:i:s', filemtime('./temp/' . $filename)) . ' GMT';
        $hash = md5($gmt);
        //$headers = getallheaders();
        
        if ((!empty($_SERVER['HTTP_IF_NONE_MATCH'])) || (!empty($_SERVER['HTTP_IF_MODIFIED_SINCE']))) {
          	if (((ereg($hash, $_SERVER['HTTP_IF_NONE_MATCH']))) || ($_SERVER['HTTP_IF_MODIFIED_SINCE'] == $gmt)) {
